...
\begin{document}
  ...
  \label{sec:intro}
    ...
    Histones are the building blocks of the nucleosome, chromatin's basic structural unit. As such,
    their structures \look{repeated `structural'} have a major impact on the chromatin dynamics,
    controlling the way genetic \look{what about epigenetic? Isn't chromatin more than just genes?}
    information can be accessed.

    \rewrite{should we have a note on nucleosome structure? It's the background of the background. Someone
    who's reading this should already know it, right?}
...
%    cluster -> gene -> transcript -> protein will be broken}
  ...
  \label{sec:nomenclature}
    ...
    the cluster taking in account mouse homologos. Some letters may be missing, e.g. HIST1H2AF, since it exists in the
...
%    \rewrite{writing is dependent on what is found}
  ...
  \section{Appendix}
    \rewrite{
      perl code
      instead of having perl code, make it really good code and submit to bioperl program section
    }
\end{document}
