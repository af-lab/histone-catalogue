...
\usepackage[textwidth=\marginparwidth + 2cm]{todonotes}                         % insert to–do and missing figure items in the document
  ...
  \newcommand{\rewrite}[2][]{\todo[inline,color=green!40,caption={chunk of text to be rewritten}#1]{#2}} % new command for boxes of text that need to be rewritten
  \newcommand{\think}[2][]{\todo[size=\small, caption={think about this}#1]{\renewcommand{\baselinestretch}{0.5}\selectfont#2\par}} % new command for long notes
...
%      of protein and gene sequence variation in canonical histones is often under-appreciated. Albig and 
    ...
    \subsection{Current situation\think{this name sucks}}
      ...
      some oversights~(Table~\ref{tab:difference-from-Marzluff02}) most likely due to the ``draft'' status of the genome it was based on.
...
%    \rewrite{writing is dependent on what is found}
  ...
  \section{Sequence extractor}
    ...
    BioPerl since version 1.6.10\think{actually, it's in the development version, next version might be 1.7 or 2.0 with the project restructure and evrything}.
  ...
  \label{sec:reproducible}
    ...
    online at \url{https://gitorious.org/human-histone-catalogue}. We hope that in the future, it can be used to create up to date information
