...
foreach my $gene (MyLib::load_canonical) {
  ...
  my $path = File::Spec->catdir($MyVar::sequences_dir, "proteins", "$access.gb");
...
foreach my $histone (keys %multi_seq) {
  my $align_path = File::Spec->catdir($MyVar::results_dir, "aligned_$histone.fasta");
  my $slogo_path = File::Spec->catdir($MyVar::figs_dir, "seqlogo_$histone.eps");
...
foreach my $histone (keys %aligned) {
  ...
  my $filepath = File::Spec->catdir($MyVar::results_dir, "table-$histone-align.tex");
